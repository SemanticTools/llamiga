...
async function complete(model, prompt, messages0, config = {}) {
  ...
  if (truncated.length > 250) {
        truncated = truncated.substring(0, 247) + ".x.";
  }
  //remove newlines
  ...
  const cannedResponses = [
    "TestBert2("+model+"): Taliho!, this is a test response to: \n"+ truncated,
    "TestBert2("+model+"): Bien, this is another test response to:\n"+ truncated,
    "TestBert2("+model+"): Mama mia, yet another test response to: \n"+ truncated,
  ];
